...
const commands = [
    ...
    new Command("help", "lists available commands, or command usage", "[cmd]", UserRoleEnum.GUEST, (player, args) => {
        let cmd = args[1];
        if (cmd && (cmd = commandMap.get(cmd))) return send(player, `usage: /${cmd.name} ${cmd.usage}`);
        send(player, "=".repeat(70));
        for (const cmd of commands)
            if (player.userRole >= cmd.minCred) send(player, `/${cmd.name} - ${cmd.description}.`)
        send(player, "=".repeat(70));
    }),
    ...
    new Command("kill", "self kill", "", UserRoleEnum.GUEST, (player, args) => {
        if (!player.cells.length) return send(player, "You cannot kill yourself, because you're not joined to the game!");
    ...
    new Command("skin", "change skin", "[skin]", UserRoleEnum.GUEST, (player, args) => {
        if (player.cells.length) return send(player, "ERROR: Cannot change skin while in game!");
    ...
    new Command("login", "upgrade your credential to access more commands", "<password>", UserRoleEnum.GUEST, (player, args) => {
        let pass = args[1];
        if (!pass || !(pass = pass.trim())) return send(player, "ERROR: missing password argument!");
        ...
        Logger.info(`LOGIN ${player.socket.remoteAddress}:${player.socket.remotePort} as "${user.name}"`);
    ...
    new Command("logout", "remove your credentials", "", UserRoleEnum.GUEST, (player, args) => {
        if (player.userRole == UserRoleEnum.GUEST) return send(player, "ERROR: not logged in");
        Logger.info(`LOGOUT ${player.socket.remoteAddress}:${player.socket.remotePort} as "${player.userAuth}"`);
    ...
    new Command("status", "shows status of the server", "", UserRoleEnum.MODER, (player, args) => {
        ...
        let humans = 0,
            bots = 0;
        for (const client of player.server.clients) client.hasOwnProperty('_socket') ? humans++ : bots++;
        send(player, "=".repeat(70));
    ...
    new Command("addbot", "adds AI bots to the server", "<count>", UserRoleEnum.ADMIN, (player, args) => {
        ...
        Logger.info(player.socket.remoteAddress + ` ADDED ${count} BOTS`);
        send(player, `Added ${count} Bots`);
    }),

    // Pause/Unpause Server
    new Command("pause", "paused/unpaused the server", "", UserRoleEnum.ADMIN, (player, args) => {
        Logger.info(`PAUSE REQUEST FROM ${player.socket.remoteAddress} as ` + player.userAuth);
        player.server.restart();
    }),

    // Restart Server
    new Command("restart", "restarts the server", "", UserRoleEnum.ADMIN, (player, args) => {
        Logger.info(`RESTART REQUEST FROM ${player.socket.remoteAddress} as ` + player.userAuth);
        player.server.restart();
    }),

    // Shutdown Serverd
    new Command("shutdown", "shutdown the server", "", UserRoleEnum.ADMIN, (player, args) => {
        Logger.info(`SHUTDOWN REQUEST FROM ${player.socket.remoteAddress} as ` + player.userAuth);
