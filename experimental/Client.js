...
const randomSkinsFile = "./experimental/randomskins.txt";
let randomSkins = [];
...
class Client {
    ...
    handshake_onCompleted(protocol, key) {
        ...
        this.sendPacket(new Packet.SetBorder(this.socket.player, this.server.border, this.server.config.serverGamemode, this.server.config.serverName + " " + this.server.config.serverVersion));
        // Send welcome message
        this.server.sendChatMessage(null, this.socket.player, this.server.config.serverName + " " + this.server.config.serverVersion);
