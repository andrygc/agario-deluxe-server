...
class Server {
    ...
    onHttpServerOpen() {
        ...
        // Done
        Logger.success("Game server started, on port " + this.config.serverPort + ".");
        Logger.success("Current game mode is " + this.mode.name + ".");
        this.loadFiles();
    ...
    start() {
        // Welcome message.
        Logger.success(`Running `+ this.config.serverName +` `+ this.config.serverVersion +`, a FOSS server implementation.`);
        ...
        if (fs.existsSync('./keys/agario-server.key') && fs.existsSync('./keys/agario-server.crt')) {
            this.httpServer = https.createServer({key: fs.readFileSync('./keys/agario-server.key', 'utf8'), cert: fs.readFileSync('./keys/agario-server.crt', 'utf8')});
            Logger.success("Using HTTPS connection, use the wss:// protocol prefix when connecting.");
        } else {
            // HTTP only
            this.httpServer = http.createServer();
            Logger.success("Using HTTP connection.");
        }
    ...
    onServerSocketError(error) {
        ...
        switch (error.code) {
            case "EADDRINUSE":
                ...
                break;
            case "EACCES":
                Logger.error("Please make sure you are running Agar.io with root privileges.");
                break;
        }
    ...
    onClientSocketOpen(ws, req) {
        ...
        Logger.info("CONNECTED " + ws.remoteAddress + ":" + ws.remotePort + ", origin: \"" + req.headers.origin + "\"");
    ...
    restart() {
        ...
        Logger.success(`Restarted ${date} ${time}`);
    }
    checkMinion(ws, req) {
    ...
    loadFiles() {        
        ...
        try {
            if (!fs.existsSync(fileNameBadWords)) {
                ...
                this.badWords = words;
                Logger.success(this.badWords.length + " bad words loaded.");
            }
        }
        catch (err) {
        ...
        try {
            ...
            this.userList = list;
            Logger.success(this.userList.length + " user records loaded.");
        }
        catch (err) {
            Logger.error(err.stack);
        ...
        try {
            if (fs.existsSync(fileNameIpBan)) {
                ...
                Logger.success(this.ipBanList.length + " IP ban records loaded.");
    ...
    startStatsServer(port) {
        ...
        this.httpServer.listen(port, function () {
            // Stats server
            Logger.success("Stats server started, on port " + port + ".");
    ...
    getStats() {
        ...
        var s = {
            ...
            'server_gamemode': this.mode.name,
            'server_chat': this.config.serverChat ? "true" : "false",
            'server_tracker': this.config.serverTracker ? "active" : "inactive",            
        ...
        this.statsObj = s;
        this.stats = JSON.stringify(s, null, 3);
    }
    ...
    pingServerTracker() {
        ...
        var data = 'current_players=' + totalPlayers +
            ...
            '&gamemode=[**] ' + this.mode.name + // we add [**] to indicate that this is Agar.io server
            ...
            '&version=Agar.io ' + this.config.serverVersion +
            '&start_time=' + this.startTime;
        trackerRequest({
            host: 'ogar.mivabe.nl',
            port: 80,
...
function trackerRequest(options, type, body) {
    ...
    options.headers['user-agent'] = 'Agar.io ' + this.config.serverVersion;
